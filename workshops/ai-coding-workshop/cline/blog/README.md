...
特に AI コーディング支援エージェントの活用においては、従来の開発環境とは異なる新たな複合的な課題が浮上しています。これらの課題は、従来の開発環境では想定されていなかった AI エージェント特有の要素を含んでいます。例えば、エージェントが"勝手に"機密情報を公開したり重要ファイルを削除するリスク、大規模なトークン消費と API 制限による開発体験への影響、といった新たな課題が存在します。これらの課題は、モデルとサービング技術の進歩により将来的に解決されることが期待されますが、現時点では適切な対応が必要です。

第 1 に、セキュリティとコンプライアンスの課題があります。従来の開発環境では、マルウェアや開発者の操作に対するセキュリティ対策が中心でしたが、AI コーディング支援エージェントでは、エージェント自体が "自律的に" 機密情報を処理・送信するリスクという新たな脅威に対応する必要があります。また、エージェントによる重要ファイルの削除など、従来とは異なる脆弱性への対策も求められます。部門やチーム単位での適切なアクセス制御の実現、セキュリティ基準への準拠と監査対応、社内ポリシーと法規制への対応など、多岐にわたる要件を満たす必要があります。

第 2 に、トークン消費と API 制限の課題があります。従来の AI チャットボットと異なり、AI コーディング支援エージェントは、コードベースの理解、複数ファイルの関連性の分析、長期的なコンテキストの維持などの処理を必要とします。そのため、1 回のタスクで数万以上のトークンを消費することも珍しくありません。また、コードの生成・レビュー・修正といった反復的な作業により、API Provider による制限（Requests Per Minute/Tokens Per Minute）への抵触リスクも高く、制限による中断は開発者の思考の流れを妨げ、開発体験の低下を招きます。

第 3 に、コスト管理の課題があります。従来の開発ツールとは異なり、AI コーディング支援エージェントのコストは使用量に応じて変動し、その予測も困難です。例えば、同じタスクでも、コードベースの複雑さ、エージェントの理解度、開発者とのやり取りの回数などによって、消費されるトークン数が大きく変動します。また、高性能なモデルほど高コストであり、適切なモデル選択の判断も必要です。部門単位での予算管理と使用状況のモニタリング、コスト効率の高いモデル選択と利用戦略の策定が求められています。チームや個人単位で自由にトークン消費を許すことで組織全体としてのコストが予測不能なレベルまで増加する可能性があるため、チームや個人単位での適切な利用制約とその可視化が必要です。

第 4 に、生産性と知識共有の課題があります。AI コーディング支援エージェントは、従来の開発環境では想定されていなかった新しい働き方をもたらします。例えば、エージェントが生成したコードの品質評価基準の確立、エージェントとの効果的な対話方法の習得、プロンプトエンジニアリングのベストプラクティス共有など、これまでにない知識やスキルの蓄積と共有が必要です。また、チーム内でのコードレビューやテスト手法の再定義、従来の開発プラクティスの再考が求められます。この課題については本記事のスコープ外としますが、組織全体での AI コーディング支援エージェント活用に関するナレッジ蓄積を意識する必要があります。

本記事では、これらの課題に対して Amazon Bedrock、LiteLLM Proxy、そして Langfuse/MLflow を組み合わせたソリューションを提案します。組織全体で効果的に AI コーディング支援エージェントを活用するための具体的な方法を検討します。
...
[AWS はセキュリティを常に最優先事項としており](https://aws.amazon.com/jp/executive-insights/content/security-first-leadership/)としており、AWS の包括的なセキュリティ・コンプライアンス対応により、安心して AI を活用するための仕組みが整っています。[Amazon Bedrock](https://aws.amazon.com/jp/bedrock/) では、入出力データが学習に利用されることはなく、モデルプロバイダーとデータが共有されない設計になっています。プロンプトログすらもデフォルトでは収集が無効化されているため、機密性の高い企業データを安全に処理できます。Amazon Bedrock は入出力に対する [Guardrails](https://aws.amazon.com/jp/bedrock/guardrails/) 機能の提供を行っており、ユーザーによる入出力データへのリスク対応が可能です。なお、海外リージョンの利用に懸念がある組織向けには、国内リージョンでのサンドボックス環境構築と、Amazon Bedrock の利用を組み合わせることで、国内リージョンに閉じて AI コーディング支援エージェントを活用することが可能です。

**ニーズ 1-1: Cline は [AWS IAM Identity Center](https://aws.amazon.com/jp/iam/identity-center/) を通じて Amazon Bedrock へのセキュアなアクセスを実現できます。しかし、開発者のローカル PC 上で Cline を利用する場合、そのツール使用（tool use）機能の性質上、完全なリスク制御が難しい側面があります。Cline は開発効率向上のために OS コマンド実行やファイル操作などの強力な機能を持ちますが、これらの機能は悪意のあるプロンプトやモデルの誤った判断により、意図しないコマンド実行や重要ファイルの削除につながる可能性があります。Cline 側での承認フローや OS 側でのアクセス権限制限などの対策は可能ですが、すべてのリスクを事前に想定することは困難です。そのため、機密情報等を保持する可能性のあるローカル PC から隔離されたサンドボックス環境の利用を推奨します。**
...
この設定ファイルでは、開発者の生産性を維持するためのモデルフォールバック機能を実装しています。高性能な Claude 3.7 Sonnet V1 を主要モデルとして設定し、このモデルがレート制限に達した場合には、自動的に Claude 3.5 Sonnet V2 にフォールバックするよう定義しています。LiteLLM Proxy のルーター設定では、デフォルトモデルとして Claude 3.7 を指定し、フォールバック機能を有効化することで、開発者が制限を意識することなく作業を継続できる環境を実現しています。この設定は必要に応じて複数の AWS アカウントやリージョンにまたがるモデルにも拡張可能です。

```yaml
# LiteLLM Proxy 設定ファイル - モデルフォールバック設定例
# このファイルは、エラー時のモデル自動切り替えに関する設定例です

model_list:
  # 第一モデル: Claude 3.7 Sonnet V1 (US Cross-Region Inference 対応)
  - model_name: bedrock-converse-us-claude-3-7-sonnet-v1
    litellm_params:
      # Amazon Bedrock のモデル識別子を指定
      model: bedrock/converse/us.anthropic.claude-3-7-sonnet-20250219-v1:0
...  
  # フォールバック用モデル: Claude 3.5 Sonnet V2 (US Cross-Region Inference 対応)
...
litellm_settings:
  # フォールバックルールの定義
  # 第一優先モデル(Claude 3.7) がレート制限に達した場合、Claude 3.5 に自動切り替え
  fallbacks: [
