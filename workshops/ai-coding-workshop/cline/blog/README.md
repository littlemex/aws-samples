...
特に AI コーディング支援エージェントの活用においては、従来の開発環境とは異なる新たな複合的な課題が浮上しています。これらの課題は、従来の開発環境では想定されていなかった AI エージェント特有の要素を含んでいます。例えば、エージェントが"勝手に"機密情報を公開したり重要ファイルを削除するリスク、大規模なトークン消費と API 制限による開発体験への影響、といった新たな課題が存在します。
