#!/bin/bash
# Performance Insights から上位の待機状態を取得するスクリプト
...
aws pi get-resource-metrics \
  ...
  --metric-queries '[{"Metric": "db.load.avg", "GroupBy": { "Group": "db.wait_state", "Limit": 7 }}]'

if [ $? -ne 0 ]; then
  echo ""
  echo "エラー: 待機状態メトリクスの取得に失敗しました。"
