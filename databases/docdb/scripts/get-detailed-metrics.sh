#!/bin/bash
# Performance Insights から詳細なパフォーマンスメトリクスを取得するスクリプト
...
aws pi get-resource-metrics \
  ...
  --metric-queries '[{"Metric": "os.memory.active.avg"}, {"Metric": "os.memory.free.avg"}, {"Metric": "os.memory.total.avg"}, {"Metric": "os.cpuUtilization.nice.avg"}, {"Metric": "os.cpuUtilization.system.avg"}]'

if [ $? -ne 0 ]; then
  echo ""
  echo "エラー: 詳細メトリクスの取得に失敗しました。"
