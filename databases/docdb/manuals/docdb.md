...
詳細は [スパイク対策 - インデックス問題ソリューション](spike-05-indexes.md) を参照してください。インデックスの診断と最適化については、[ユーザー診断ガイド](spike-10-diagnostics.md#インデックスビルド進捗モニタリング)も併せて参照してください。
...
- 実装: 新規インデックスの作成（Amazon DocumentDB 4.0/5.0ではマルチワーカー対応）
...
- インデックスの種類や設定を見直し
- 実装: インデックス戦略の総合的な見直し（Amazon DocumentDB 5.0では`reIndex`コマンドによるメンテナンスも可能）
...
Amazon DocumentDBのパフォーマンス分析と監視のための公式ツールセットについては、[パフォーマンス分析ツールガイド](docdb-perf-tools.md)を参照してください。このツールセットには以下が含まれます：

### パフォーマンス分析ツール
- インデックス分析（index-review）: コレクションとインデックスの構造・使用状況を包括的に分析
- メトリクス収集（metric-collector）: クラスターとインスタンスの詳細なメトリクスを収集
- データ圧縮分析（compression-review）: コレクションのデータ圧縮率を計算
- デプロイメントスキャン（deployment-scanner）: クラスターの構成を分析しコスト最適化を提案
- インデックスカーディナリティ検出（index-cardinality-detection）: インデックスの選択性を分析

### モニタリングツール
- リアルタイムコレクション監視（documentdb-top）: コレクションレベルのメトリクスをリアルタイムで表示
- インスタンス統計表示（docdb-stat）: インスタンスレベルの統計情報を表示
- ダッシュボード作成（docdb-dashboarder）: CloudWatchダッシュボードを自動生成
- GC監視（gc-watchdog）: ガベージコレクションのアクティビティを追跡
- カスタムメトリクス収集: 独自のメトリクスをCloudWatchに送信

これらのツールは、スパイク対策や定常的な負荷対策の各フェーズで活用できます。詳細な使用方法、インストール手順、およびベストプラクティスについては、[パフォーマンス分析ツールガイド](docdb-perf-tools.md)を参照してください。
