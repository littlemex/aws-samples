...
さらに、従来のBIプラットフォームには以下のような課題があります：
...
- **遅く静的なインサイト**: 従来の BI システムは、静的なダッシュボードと事前設定されたレポートを中心に設計されています。リアルタイムのビジネス課題に対応するために必要な動的でその場で得られるインサイトを提供できません。
- **高い所有コスト**: トレーニング、メンテナンス、IT の関与の組み合わせにより、従来の BI ツールの総所有コストが大幅に増加し、組織にとってスケーラビリティが低下します。

## Wren AI とは

Wren AI のビジョンと設計をより深く掘り下げると、私たちはビジネスインテリジェンスの次のフロンティアを定義する境界線に立っています。それは「生成型 BI」または「GenBI」と呼ぶものです。GenBI は、BI ツールとの対話方法だけでなく、これらのシステムが根本的にどのように構築されているかを再考するパラダイムシフトを意味します。これはインターフェースの単なる進化を超え、BI アーキテクチャの核心の完全な再発明です。この変革の中心には、GenBI がデータ駆動型意思決定の未来を再定義できるようにする 4 つの基盤レイヤーがあります。ここが BI の未来の始まりであり、私たちはその先頭に立つことに興奮しています。
...
**データレイヤー**  
この基盤は、SaaS ツール、データベース、クラウドストレージなどの多様なソースからのデータを統合し、統一します。コンポーザブルアーキテクチャに基づいており、このレイヤーは柔軟性とスケーラビリティを提供します。

**セマンティックレイヤー**  
セマンティックレイヤーはデータポイント間の関係をマッピングし、多様なソース間でデータの意味を統一することでコンテキストを考慮したインサイトを確保します。

**エージェントレイヤー**  
データとの対話的で人間のような会話を可能にする会話エンジン。AI エージェントはあなたのクエリを理解し、意図を解釈し、コンテキストに応じて応答し、シームレスな改良とフォローアップを可能にします。
