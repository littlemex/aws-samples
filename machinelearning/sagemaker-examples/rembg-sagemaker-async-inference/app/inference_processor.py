...
class InferenceProcessor(ABC):
    ...
    def __init__(self, model_name: str = 'u2net', use_cloudwatch: bool = True):
        ...
        try:
            ...
            logger.error(f"Failed to prepare model files during initialization: {str(e)}")
            raise
    ...
    def _create_session(self):
        try:
            ...
            logger.info(f"Successfully created RembgHandler for model: {self.model_name}")
    ...
    def _check_models(self) -> str:
        ...
        if not model_path.exists():
            if tar_path.exists():
                ...
                logger.error(f"Neither model file nor tar archive found: {model_path}, {tar_path}")
                raise FileNotFoundError(f"Neither model file nor tar archive found: {model_path}, {tar_path}")
        else:
    ...
    def process_image(self, image_data: bytes) -> bytes:
        ...
        try:
            ...
            logger.info(f"Input image opened successfully: size={image.size}, mode={image.mode}")
